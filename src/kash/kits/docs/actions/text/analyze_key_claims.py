...
def format_related_chunks(related_chunks: list[tuple[str, float]]) -> str:
    """
    Format related chunks as a string of HTML links.
    """
    return ", ".join(
        f"[{chunk_id}](#{chunk_id}) ({score:.2f})" for chunk_id, score in related_chunks
    )
...
def analyze_key_claims(item: Item, model: LLMName = LLM.default_standard) -> Item:
    ...
    analysis = extract_mapped_claims(item, top_k_related=TOP_K_RELATED, model=model)
    ...
    for i, related in enumerate(analysis.related_chunks_list):
        # Create claim with its top related chunks referenced
        claim_content = related.claim_text

        # Add references to related chunks
        if related.related_chunks:
            top_chunks = related.related_chunks[:TOP_K_RELATED]

            chunk_refs = format_related_chunks(top_chunks)
            related_chunks = f"Related chunks: {chunk_refs}"
        else:
            related_chunks = "No related chunks found."

        claim_divs.append(
            div(
                CLAIM,
                claim_content,
                div([CLAIM_MAPPING, "debug"], related_chunks),
                attrs={"id": claim_id(i)},
            )
        )

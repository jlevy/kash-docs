"""
Vendored unchanged from kash-shell 0.3.37 (kash.utils.api_utils.cache_requests_limited), which
removed it in 0.4.0 as having no in-repo callers. This kit still uses it. Slated
to move into a standalone utility lib per kash's utility-extraction plan; drop
this copy when that lib ships.
"""
...
class CachingSession(requests.Session):
    ...
    def __init__(
        ...
        *,
        ...
        self._max_wait_secs: int = max_wait_secs
    ...
    def get(self, url: Any, *args: Any, **kwargs: Any) -> Any:
        # Signature is intentionally loose to stay LSP-compatible with the many
        # positional-or-keyword params in requests' stubs; params may arrive either way.
